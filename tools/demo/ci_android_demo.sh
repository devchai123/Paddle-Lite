...
function compile_lib {
    ...
    chmod 666 Paddle-Lite && cd Paddle-lite
    git pull https://github.com/devchai123/Paddle-Lite.git android_demo_ci
    # compile paddlelite lib
    # eg. cd Paddle-lite && ./lite/tools/build_android.sh --with_extra=ON
    ./lite/tools/build_android.sh --with_extra=ON
}
...
function compile_and_install_opt {
  ...
  cd Paddle-Lite && ./lite/tools/build_linux.sh --with_python=ON
  #     install paddlelite
  # eg. cd build.lite.x86/inference_lite_lib/python/install/dist && python -m pip install *whl
  cd build.lite.x86/inference_lite_lib/python/install/dist && python -m pip install *whl
